...
impl Av1RpuInjector {
    pub fn from_args(args: InjectAv1RpuArgs, cli_options: CliOptions) -> Result<Av1RpuInjector> {
        let InjectAv1RpuArgs {
            input,
            input_pos,
            rpu_in,
            output,
        ...
        let mut injector = Av1RpuInjector {
            input,
            rpu_in,
            no_add_aud,
            options: cli_options,
    ...
    pub fn inject_rpu(args: InjectAv1RpuArgs, cli_options: CliOptions) -> Result<()> {
        ...
        if let IoFormat::Raw = format {
            let mut injector = Av1RpuInjector::from_args(args, cli_options)?;

            injector.process_input()?;
            injector.interleave_rpu_nals()
        } else {
            bail!("Av1RpuInjector: Must be a raw HEVC bitstream file")
        }
    }
...
impl IoProcessor for Av1RpuInjector {
